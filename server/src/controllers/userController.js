...
const authenticateUser = async (req, res) => {
  ...
  if (result.isValid) {
    const { name, id, email, img_avatar: imgAvatar } = result.user;
    const userData = { name, email, id, imgAvatar };
...
const addFavoriteUser = async (req, res) => {
  try {
    ...
    if (userId === userToAddId) {
      return res.status(400).json({ message: 'No puedes agregarte a ti mismo a favoritos' });
    }
    ...
    if (user.favoriteUsers.includes(userToAddId)) {
      return res.status(400).json({ message: 'Este usuario ya está en tus favoritos' });
    }
    ...
    return res.status(200).json({ message: 'Usuario agregado a favoritos correctamente' });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Error en el servidor' });
  }
...
const removeFavoriteUser = async (req, res) => {
  try {
    ...
    if (userId === userToRemoveId) {
      return res.status(400).json({ message: 'No puedes eliminarte a ti mismo de favoritos' });
    }
    ...
    if (!user.favoriteUsers.includes(userToRemoveId)) {
      return res.status(400).json({ message: 'Este usuario no está en tus favoritos' });
    }
    ...
    return res.status(200).json({ message: 'Usuario eliminado de favoritos correctamente' });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Error en el servidor' });
  }
